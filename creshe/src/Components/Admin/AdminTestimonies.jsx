...
export default function AdminTestimonies() {
  ...
  const { data, isLoading } = useSWR('https://backend-creshe.onrender.com/api/testimonies', fetcher);
  ...
  const handleAddTestimony = async (e) => {
    ...
    try {
      const response = await fetch('https://backend-creshe.onrender.com/api/testimonies', {
        method: 'POST',
        headers: {
          ...getAuthHeaders(),
      ...
      setRating(5);
      setImageUrl('');
      mutate('https://backend-creshe.onrender.com/api/testimonies');
    } catch (error) {
  ...
  const deleteTestimony = async (id, testimonyName) => {
    ...
    try {
      const response = await fetch(`https://backend-creshe.onrender.com/api/testimonies/${id}`, {
        method: 'DELETE',
