...
export default function AdminTestimonies() {
  ...
  const { data, isLoading } = useSWR('http://localhost:5000/api/testimonies', fetcher);
  ...
  const handleAddTestimony = async (e) => {
    ...
    try {
      ...
      setRating(5);
      setImageUrl('');
      mutate('http://localhost:5000/api/testimonies');
    } catch (error) {
  ...
  const deleteTestimony = async (id, testimonyName) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/testimonies/${id}`, {
        method: 'DELETE',
